...
if leaf_img is None or mask_img is None:
    ...
    # Calculate zoom metrics early so they apply to all click handlers
    zoom = {"4x (Scanning Objective)": 1.0, "10x (Low Power Objective)": 2.2, "40x (High Power Objective)": 5.0}[objective_lens] if 'objective_lens' in locals() else 2.2
    
    # We need objective_lens defined before layout; let's pull it from the controls column first or set default
